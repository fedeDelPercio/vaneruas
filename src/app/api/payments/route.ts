...
export async function GET(req: NextRequest) {
  ...
  const { data: convs } = convIds.length
    ? await sb.from("conversations").select("id, display_name, source, contact_email, external_id").in("id", convIds)
    : { data: [] };
  ...
  if (missingConvIds.length) {
    const { data: moreConvs } = await sb
      .from("conversations")
      .select("id, display_name, source, contact_email, external_id")
